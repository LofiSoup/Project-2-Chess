...
main :-
  set_random(seed(random)),  % Setting up the seed for random number generator
    init_board(InBoard),
...
/* ----------------------------------------------------------------------- */
/* Printing the board visually */
/* Printing the board visually with borders */
/* Printing the board visually with borders */
/* Printing the board visually with wider columns for two-character pieces */
print_board(Board) :-
  nl, write('  +----+----+----+----+----+----+----+----+'), nl,  % Top border
  print_ranks(8, Board), nl,
  write('    a    b    c    d    e    f    g    h'), nl.  % Column labels

% Print each rank from 8 down to 1
print_ranks(0, _) :- !.  % Stop when rank reaches 0
print_ranks(Rank, Board) :-
  format('~d |', [Rank]),  % Print rank number and left border
  print_files(a, Rank, Board),  % Print files (columns) for this rank
  format(' ~d', [Rank]), nl,  % Print rank number on the right side
  write('  +----+----+----+----+----+----+----+----+'), nl,  % Row border
  NextRank is Rank - 1,  % Move to next rank down
  print_ranks(NextRank, Board).

% Print files (columns) a to h for a given rank
print_files(i, _, _) :- !.  % Stop when file goes beyond h
print_files(File, Rank, Board) :-
  (   member(piece(File-Rank, Color, Type), Board)
  ->  piece_symbol(Color, Type, Symbol)
  ;   Symbol = '  '  % Empty square with 2 spaces
  ),
  format(' ~w |', [Symbol]),  % Ensure each piece or square is printed between borders with 2 spaces
  next_file(File, NextFile),
  print_files(NextFile, Rank, Board).

% Map piece types to their symbols (2 characters wide)
...
% Helper to move to the next file
next_file(a, b).
next_file(b, c).
next_file(c, d).
next_file(d, e).
